...
def process_coordinates(start_time: int, end_time: int) -> List[Dict[str, str]]:
    ...
    for departure in api_response:
        ...
        # Checking for any duplicates before appending
        dictionary.append([{departure_airport: [lon_dep, lat_dep]}, {arrival_airport: [lon_arr, lat_arr]}])
    # return json.dumps(_data)

    new_dictionary = set()
    for i in dictionary:
        new_dictionary.add(str(i))
...
def process_flights(start_time: int, end_time: int) -> List[Dict[str, str]]:
    ...
    for departure in api_response:
        ...
        dictionary.append({"departure airport": departure_airport, "arrival airport": destination_airport})
...
def index() -> str:
    ...
    nowt = strftime("%H:%M:%S", gmtime())
    current_time = nowt
    current_date = now.strftime('%B %d, %Y')
    return f"Hello world!, the current_time is {current_time}, and current_date is {current_date}"
